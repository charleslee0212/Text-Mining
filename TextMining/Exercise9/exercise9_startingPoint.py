...
wordsPositive = set()
fpp = open("positive.txt",mode='r',encoding="ISO-8859-1")
for line in fpp:
	aStr = line.replace('\n','')
...
wordsNegative = set()
fpn = open("negative.txt",mode='r',encoding="ISO-8859-1")
for line in fpn:
	aStr = line.replace('\n','')
	line.replace('\n','')
...
while (iterationCount < 200000) and (avgGrad() > 0.001) :
	...
	doBatchIteration(eta = globalETA)

...
y_train = labelPosDocs + labelNegDocs


# create the feature vectors using CountVectorizer
cv = CountVectorizer(binary=False,max_df=0.95)
cv.fit_transform(docs)
x_train = cv.transform(docs)   # counts is now a list of feature vectors
...
for line in fpPosTest:
	posTest.append(line)
    
fpNegTest = open('negTest.txt', 'r') 
for line in fpNegTest:
	negTest.append(line)
    
# create lists of labels for the documents
labelPosTest = [1 for i in posTest]
labelNegTest = [0 for i in negTest]

test = posTest + negTest

y_test = labelPosTest + labelNegTest
